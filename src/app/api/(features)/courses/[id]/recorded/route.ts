...
export async function POST(
  ...
  try {
    ...
    // Optional: Verify that this instructor owns the course
    // const course = await courseService.getCourseById(parseInt(id));
    // if (course?.instructor.id !== session.id) return ...

    const recordedClass = await courseService.addRecordedClass(parseInt(id), body);
    return NextResponse.json(recordedClass, { status: 201 });
  } catch (error: any) {
